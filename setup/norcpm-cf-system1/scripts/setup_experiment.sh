...
do
  ...
  do
    ...
    if [ $COUNT -eq `expr $MEMBER - $MEMBER1 + 1` ] 
    then 
      REF_YEAR=$YEAR
      REF_MONTH=$MONTH
      REF_DAY=$DAY
  ...
  CASE1=${ENSEMBLE_PREFIX1}_${MEMBERTAG}${MEMBER1}
  ...
  then 
    ...
    then
      ...
      then
        if (( $REF_ENSEMBLE ))
        then
          REST_PATH=$REST_PATH_LOCAL/${REST_CASE}/rest/${REF_YEAR}-${REF_MONTH}-${REF_DAY}_${MEMBERTAG}${REF_MEMBER}
        else
          REST_PATH=$REST_PATH_LOCAL/${REST_CASE}/rest/${REF_YEAR}-${REF_MONTH}-${REF_DAY}-00000
        fi
        if [ ! -e $REST_PATH ]
        then
      ...
      REF_MEMBER=`echo 0\`expr $MEMBER + $REF_MEMBER1 - $MEMBER1\` | tail -3c`
      REST_CASE=${REST_PREFIX}${REF_MEMBER}
      REST_PATH=$REST_PATH_LOCAL/${REST_CASE}/${START_YEAR}-${START_MONTH}-${START_DAY}
      if [ ! -e $REST_PATH ]
      then
        echo cannot locate restart data in $REST_PATH  
        REST_PATH=$REST_PATH_LOCAL/${REST_CASE}/rest/${START_YEAR}-${START_MONTH}-${START_DAY}-00000
        if [ ! -e $REST_PATH ]
        then
          echo cannot locate restart data in $REST_PATH  
          exit
    ...
    ./xmlchange -file env_build.xml -id EXEROOT -val $EXEROOT
    ./xmlchange -file env_run.xml -id DOUT_S_ROOT -val $DOUT_S_ROOT
    ./xmlchange -file env_conf.xml -id RUN_REFCASE -val ${REST_PREFIX}${REF_MEMBER}
    ./xmlchange -file env_conf.xml -id GET_REFCASE -val FALSE
    if [[ $RUN_TYPE && $RUN_TYPE == "hybrid" ]]
    then
      ./xmlchange -file env_conf.xml -id RUN_TYPE -val hybrid
    ...
    then
      ...
      if (( $REF_ENSEMBLE ))
      then
        REST_PATH=$REST_PATH_LOCAL/${REST_CASE}/${REF_YEAR}-${REF_MONTH}-${REF_DAY}_${MEMBERTAG}${REF_MEMBER}
      else
        REST_PATH=$REST_PATH_LOCAL/${REST_CASE}/${REF_YEAR}-${REF_MONTH}-${REF_DAY}
      fi
      if [ ! -e $REST_PATH ]
      ...
      sed -i "s%${ENSEMBLE_PREFIX1}/${CASE1}/run/${REST_PREFIX}${REF_MEMBER1}.cam2.r.${START_YEAR1}-${START_MONTH1}-${START_DAY1}%${ENSEMBLE_PREFIX}/${CASE}/run/${REST_PREFIX}${REF_MEMBER}.cam2.r.${START_YEAR}-${START_MONTH}-${START_DAY}%" Buildconf/cam.buildnml.csh 
      sed -i "s%${REST_PREFIX}${REF_MEMBER1}.cice.r.${START_YEAR1}-${START_MONTH1}-${START_DAY1}%${REST_PREFIX}${REF_MEMBER}.cice.r.${START_YEAR}-${START_MONTH}-${START_DAY}%" Buildconf/cice.buildnml.csh
      sed -i "s%${REST_PREFIX}${REF_MEMBER1}.clm2.r.${START_YEAR1}-${START_MONTH1}-${START_DAY1}%${REST_PREFIX}${REF_MEMBER}.clm2.r.${START_YEAR}-${START_MONTH}-${START_DAY}%" Buildconf/clm.buildnml.csh
    fi 
