...
! Sep  2011, modified by Øyvind Seland and Alf Kirkevåg, September, based on NorESM/CCSM4 code. 
...
!#define DIAGNCDNC
...
#ifdef DIRIND
    use phys_buffer,     only:  pbuf_fld, pbuf_old_tim_idx, pbuf_get_fld_idx
#else
...
#ifdef DIRIND
    use aerosoldef
    use opttab,           only: nbands, nmodes, eps
...
#ifdef DIRIND
    real(r8) emisx(pcols,pver)                 ! aerosol-calculated offline Cloud longwave emissivity
#endif
...
#ifdef DIRIND
    real(r8) :: cliqwx(pcols,pver)             ! aerosol-calculated offline in-cloud cloud liquid water path
#endif
    ...
    real(r8) :: volcmmr(pcols,pver)     ! Stratospheric aerosol mmr, 0 if not prescribed              
    integer ifld
#endif
...
#ifdef DIRIND  
!when taken in:    real(r8), intent(in) :: relca(pcols,pver) ! computed effective radius of liquid clouds
    real(r8) :: relca(pcols,pver) ! computed effective radius of liquid clouds
!#define DIAGNCDNC
#ifndef DIAGNCDNC
!when taken in:    real(r8), intent(in) :: fochana(pcols) ! frequency of occurence of "Han" clouds
    real(r8) :: fochana(pcols) ! frequency of occurence of "Han" clouds
    real(r8) :: cdncint(pcols) ! cloud droplet burden
#endif
! Local variables used for calculating aerosol optics and direct and indirect forcings
    ...
    real(r8) clearodvis(pcols), clearabsvis(pcols), cloudfree(pcols)
    ...
    real(r8) ssatot(pcols,pver,nbands)  ! aerosol single scattering albedo
    real(r8) asymtot(pcols,pver,nbands) ! aerosol asymmetry factor
    real(r8) betot(pcols,pver,nbands)   ! aerosol extinction
    ...
    integer ipar                      ! new aerosol parameterization on(1)/off(0)
...
!----------------------------------------------------------------------
    ...
    if (has_prescribed_volcaero) then
      ifld = pbuf_get_fld_idx('VOLC_MMR')
      rvolcmmr=>pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
      volcmmr(:,:)=rvolcmmr(:,:)
    end if
#endif
...
!t      write(*,*) 'latj, latj+1, area =', slatj, slatjp1, gridarea(i)
    end do
...
#ifdef DIRIND
            cicewp, cliqwp, cliqwx, emis, emisx, cldtau, rel, rei, pmxrgn, nmxrgn, snowh, pbuf)
#else
...
#ifdef DIRIND
               aer_tau, aer_tau_w, aer_tau_w_g, aer_tau_w_f, rhtrunc)
#else
...
!cak+  Calculate NorESM aerosol optical parameters
          ipar=1  
          iant=1
          call pmxsub(lchnk,  ncol,    pnm,     state%pmid, iant,  &
                      coszrs, state%t, qdirind, rhtrunc,    Nnatk, &
                      ssatot, asymtot, betot,   Ctot,       deltah_km, & 
                      aodvis, absvis,  volcmmr)

!      Replace CCSM4 standard aerosol optics with NorESM optics (except top layer: no aerosol)
!      (better to move these loops to pmxsub?)
       do i=1,ncol
        do ns=1,19
            per_tau(i,0,ns)= 0._r8
            per_tau_w(i,0,ns)= 0._r8
            per_tau_w_g(i,0,ns)= 0._r8
            per_tau_w_f(i,0,ns)= 0._r8
        end do
       end do
       do i=1,ncol
        do ns=1,10   ! band 10 is slingo band 2 (0.69-1.19um)
          do k=1,pver
            per_tau(i,k,ns)=deltah_km(i,k)*betot(i,k,ns)
             per_tau_w(i,k,ns)=per_tau(i,k,ns)*max(min(ssatot(i,k,ns),0.999999_r8),1.e-6_r8)
            per_tau_w_g(i,k,ns)=per_tau_w(i,k,ns)*asymtot(i,k,ns)
            per_tau_w_f(i,k,ns)=per_tau_w_g(i,k,ns)*asymtot(i,k,ns)
          end do
        end do
        do ns=11,14  ! using slingo band 3 (1.19-2.38um)
          do k=1,pver
            per_tau(i,k,ns)=deltah_km(i,k)*betot(i,k,11)
             per_tau_w(i,k,ns)=per_tau(i,k,ns)*max(min(ssatot(i,k,11),0.999999_r8),1.e-6_r8)
            per_tau_w_g(i,k,ns)=per_tau_w(i,k,ns)*asymtot(i,k,11)
            per_tau_w_f(i,k,ns)=per_tau_w_g(i,k,ns)*asymtot(i,k,11)
          end do
        end do
        do ns=15,19  ! using slingo band 4 (2.38-4.00um)
          do k=1,pver
            per_tau(i,k,ns)=deltah_km(i,k)*betot(i,k,12)
             per_tau_w(i,k,ns)=per_tau(i,k,ns)*max(min(ssatot(i,k,12),0.999999_r8),1.e-6_r8)
            per_tau_w_g(i,k,ns)=per_tau_w(i,k,ns)*asymtot(i,k,12)
            per_tau_w_f(i,k,ns)=per_tau_w_g(i,k,ns)*asymtot(i,k,12)
          end do
        end do
       end do
!       write(*,*) 'ptau_f(1,26,8) =', per_tau_w_f(1,26,8) 

#undef AEROFFL ! test
!#define AEROFFL ! test
#ifdef AEROFFL   ! for calculation of direct and indirect radiative forcing (direct here) 

       pmxrgnrf(1:ncol,:) = pmxrgn(1:ncol,:)
       nmxrgnrf(1:ncol)   = nmxrgn(1:ncol)

          call t_startf('radcswmx_drf')

!         Call SW radiation code with NorESM optics but std. CCSM4 cloud droplet scheme
          call radcswmx(lchnk, &
               ncol,       pnm,        pbr,        sp_hum,     o3,         &
               o2_col,     cld,        cicewp,     cliqwp,     rel,        &
               rei,        eccf,       coszrs,     solin,      &
               cam_in%asdir, cam_in%asdif, cam_in%aldir, cam_in%aldif, nmxrgnrf, &
               pmxrgnrf,     qrs,        qrsc,       fsnt,       fsntc,    fsdtoa, &
               ...
               per_tau,    per_tau_w,  per_tau_w_g, per_tau_w_f, liq_icld_vistau, ice_icld_vistau )

          call t_stopf('radcswmx_drf')
             ...
             call outfld('FSUS_DRF',fsds-fsns,pcols,lchnk)
       ...
       nmxrgnrf(1:ncol)   = nmxrgn(1:ncol)
          ...
          call t_startf('radcswmx_aie')

!         Call of SW radiation code with NorESM cloud droplet scheme (1.AIE & 2.AIE) but std. CCSM4 aerosol optics
!         Note: only forcing for 1.AIE is (so far) available with the diagnostic CDNC scheme
          call radcswmx(lchnk, &
               ncol,       pnm,        pbr,        sp_hum,     o3,         &
#ifdef DIAGNCDNC
               o2_col,     cld,        cicewp,     cliqwp,     relca,        &
#else !prognostic CDNC
               o2_col,     cld,        cicewp,     cliqwx,     relca,        &
#endif
               rei,        eccf,       coszrs,     solin,      &
               cam_in%asdir, cam_in%asdif, cam_in%aldir, cam_in%aldif, nmxrgnrf, &
               pmxrgnrf,     qrs,        qrsc,       fsnt,       fsntc,    fsdtoa, &
          ...
          call t_stopf('radcswmx_aie')
             ...
             ! Dump shortwave radiation information to history tape buffer (diagnostics)
             !
             call outfld('QRS_AIE ',ftem  ,pcols,lchnk)
             ftem(:ncol,:pver) = qrsc(:ncol,:pver)/cpair
             call outfld('QRSC_AIE',ftem  ,pcols,lchnk)
             call outfld('FSNT_AIE',fsnt  ,pcols,lchnk)
             call outfld('FSNS_AIE',fsns  ,pcols,lchnk)
             call outfld('FSNTCAIE',fsntc ,pcols,lchnk)
             call outfld('FSNSCAIE',fsnsc ,pcols,lchnk)

             do i=1,ncol
                swcf(i)=(fsntoa(i) - fsntoac(i))*cgs2mks
             end do

#else  ! ONLINE simulations, no separate aerosol radiative forcing calculations
          call t_startf('radcswmx_aer')

!         Call SW radiation code with NorESM optics and cloud droplet scheme (1.AIE & 2.AIE)
          call radcswmx(lchnk, &
               ncol,       pnm,        pbr,        sp_hum,     o3,         &
#ifdef DIAGNCDNC
               o2_col,     cld,        cicewp,     cliqwp,     relca,      &
          ...
          call t_stopf('radcswmx_aer')
#endif  ! aeroffl

#ifdef AEROFFL   ! for calculation of direct and indirect radiative forcing (indirect here)  

          call t_startf('radcswmx')

!         Last call of SW radiation code with std CCSM4 aerosol optics and cloud droplet properies
          call radcswmx(lchnk, &
               ncol,       pnm,        pbr,        sp_hum,     o3,         &
               o2_col,     cld,        cicewp,     cliqwp,     rel,        &
               rei,        eccf,       coszrs,     solin,      &
...
#else   ! not DIRIND, i.e. with std CCSM4 aerosols and cloud droplet properties
...
#ifdef DIRIND
#ifdef DIAGNCDNC
             cfc11, cfc12, cld, emis, pmxrgn, &       ! no LW effect with DIAGNCDNC (only test use) 
#else
             cfc11, cfc12, cld, emisx, pmxrgn, &
#endif
#else
...
#ifdef AEROFFL
          do i = 1, ncol
             flnt_tmp(i)  = flnt(i) *cgs2mks
          end do
          call outfld('FLNT_AIE',flnt_tmp,pcols,lchnk)

          do i=1,ncol
             lwcf(i)=(flutc(i) - flut(i))*cgs2mks
          end do
...
!flyttes?             swcf(i)=fsntoa(i) - fsntoac(i)
#endif
          end do
...
#ifdef AEROCOM
          call outfld('FLUS    ',flds-flns,pcols,lchnk)
#endif
...
!        Total and absorption optical depth under cloudfree conditions
       do i = 1, ncol
         cloudfree(i)=1._r8-cltot(i)
         clearodvis(i)=cloudfree(i)*aodvis(i)
         clearabsvis(i)=cloudfree(i)*absvis(i)
       end do
       call outfld('CDODVIS ',clearodvis,pcols,lchnk)
       ...
       call outfld('CDNCINT ',cdncint,pcols,lchnk)
#endif
#endif
